...
export const getBitcoinBasic = async () => await axios
  .get(`${baseUrl}${basicDataUrlAdditive}`)
  .then(({data}) => {
    const {
      categories,
      genesis_date,
...
export const getBitcoinMarket = async (start, end) => {
  console.log(start, end);
  await axios
    .get(`${baseUrl}${marketUrlAdditive}${currencyUrlAdditive}${daterangeUrlAdditive(start, end)}`)
    .then(({data}) => {
      const {prices} = data;
      const marketvalues = prices.map((mv) => ({time: mv[0], price: mv[1]}));
